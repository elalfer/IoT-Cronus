...
void valveControl::add_to_list(icaltimetype start, icaltimetype end)
{
    struct icaltime_span ts = icaltime_span_new(start,end,1);
	this->events.push_back(ts);
}

void valveControl::sort_events()
{

}

bool valveControl::IsActive() {
    time_t now = time(0);
...
int valveControl::ParseICALFromString(const string &ical)
{
...
int valveControl::ParseICALFromFile(string file_name)
{
